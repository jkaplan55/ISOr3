...
* On Windows: Copy both .dll's in the "shared libs" into your max folder (where max.exe lives).  The **release** .dll must also be included in the `resources\support` folder of any standalones that use the externals.
...
## Creating a Max Standalone that Contains the Externals
